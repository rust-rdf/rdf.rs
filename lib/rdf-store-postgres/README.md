...
```toml
[dependencies]
rdf-store-postgres = { version = "0.4" }
```
...
```toml
[dependencies]
rdf-store-postgres = { version = "0.4", default-features = false, features = ["tracing"] }
```
...
the underlying [`PostgresStore`] quad store into a [`SparqlStore`]:

```rust,compile_fail
use sparql_store::SparqlStore;

let mut store: SparqlStore<PostgresStore> = ... // TODO
