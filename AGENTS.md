...
- Preserve RDF term kind, lexical form, datatype, language, direction, and graph through conversions. A quad's `None` context denotes the default graph; a pattern's `None` slot is a wildcard and `Some(DEFAULT_GRAPH.into())` selects the default graph. Compare complete terms, not just `value_str()`; the singleton is distinct from its external `urn:rdf:default-graph` IRI.
